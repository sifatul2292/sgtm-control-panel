...
function tagiooTikTokEventsApiTemplateData() {
  return `___TERMS_OF_SERVICE___
...

{
  ...
  "version": 1,
  "securityGroups": [],
  "displayName": "Tagioo TikTok Events API",
  "categories": ["ADVERTISING", "ANALYTICS", "CONVERSIONS"],
  "brand": {
    "id": "tagioo",
    "displayName": "Tagioo"
  },
  "description": "Sends GA4-client server events from Tagioo Server GTM to TikTok Events API.",
  "containerContexts": ["SERVER"]
}
...
[
  ...
  },
  {
    "type": "TEXT",
    "name": "accessToken",
    "displayName": "TikTok Events API Access Token",
    "simpleValueType": true,
    "valueValidators": [{ "type": "NON_EMPTY" }]
  },
  {
    "type": "CHECKBOX",
    "name": "enableDebugLog",
    "checkboxText": "Log TikTok response in Server GTM preview",
    "simpleValueType": true,
    "defaultValue": true
  }
]
...
function hash(value, lowerCase) {
  if (value === undefined || value === null || value === '') return undefined;
  let stringValue = makeString(value).trim();
  if (lowerCase) stringValue = stringValue.toLowerCase();
  if (!stringValue) return undefined;
  if (stringValue.match('^[A-Fa-f0-9]{64}$') !== null) return stringValue;
  return sha256Sync(stringValue, { outputEncoding: 'hex' });
}

function addHashed(target, key, value, lowerCase) {
  const hashed = hash(value, lowerCase);
  if (hashed !== undefined) target[key] = hashed;
}
...
const mappedEventName = eventMap[eventData.event_name];
if (!mappedEventName) {
  if (data.enableDebugLog) logToConsole('Tagioo TikTok Events API: skipping unmapped event ' + eventData.event_name);
  data.gtmOnSuccess();
  return;
}
...
const userData = eventData.user_data || {};
const user = {};
addHashed(user, 'email', firstValue(userData.email_address, userData.email, eventData.email_address, eventData.email), true);
addHashed(user, 'phone', firstValue(userData.phone_number, eventData.phone_number), true);
addHashed(user, 'external_id', firstValue(userData.external_id, eventData.external_id, eventData.user_id), false);
addRaw(user, 'ttclid', firstValue(eventData.ttclid, getRequestHeader('x-tt-click-id'), getCookieValues('tagioo_ttclid', true)[0]));
addRaw(user, 'ttp', firstValue(eventData.ttp, getCookieValues('_ttp', true)[0]));
addRaw(user, 'ip', firstValue(eventData.ip_override, eventData.ip, getRequestHeader('x-forwarded-for'), getRequestHeader('x-real-ip')));
addRaw(user, 'user_agent', firstValue(eventData.user_agent, getRequestHeader('user-agent')));

const properties = { event_trigger_source: 'TagiooServerGTM' };
if (eventData.currency) properties.currency = makeString(eventData.currency).toUpperCase();
if (eventData.value !== undefined && eventData.value !== null && eventData.value !== '') {
  properties.value = makeNumber(eventData.value);
}
if (getType(eventData.items) === 'array') {
  properties.contents = eventData.items.map((item) => ({
    content_id: makeString(firstValue(item.item_id, item.id, item.item_name) || ''),
    content_name: makeString(firstValue(item.item_name, item.name) || ''),
...
const body = {
  ...
  partner_name: 'TagiooServerGTM',
  data: [event]
};

sendHttpRequest('https://business-api.tiktok.com/open_api/v1.3/event/track/', {
  method: 'POST',
  headers: {
    'Content-Type': 'application/json',
  ...
  if (data.enableDebugLog) {
    logToConsole('Tagioo TikTok Events API response: ' + response.statusCode + ' ' + response.body);
  }
  if (response.statusCode >= 200 && response.statusCode < 300) data.gtmOnSuccess();
...
[
  ...
  },
  {
    "instance": {
      ...
      "param": [
        { "key": "allowedUrls", "value": { "type": 1, "string": "specific" } },
        { "key": "urls", "value": { "type": 2, "listItem": [{ "type": 1, "string": "https://business-api.tiktok.com/" }] } }
      ]
    },
    "isRequired": true
  },
  {
    "instance": {
      ...
      "param": [
        { "key": "cookieAccess", "value": { "type": 1, "string": "specific" } },
        { "key": "cookieNames", "value": { "type": 2, "listItem": [{ "type": 1, "string": "tagioo_ttclid" }, { "type": 1, "string": "_ttp" }] } }
      ]
    },
    "isRequired": true
  },
      ...
      "param": [
        ...
        { "key": "headersAllowed", "value": { "type": 8, "boolean": true } },
        {
          ...
          "value": {
            "type": 2,
            "listItem": [
              { "type": 3, "mapKey": [{ "type": 1, "string": "headerName" }], "mapValue": [{ "type": 1, "string": "x-tt-click-id" }] },
              { "type": 3, "mapKey": [{ "type": 1, "string": "headerName" }], "mapValue": [{ "type": 1, "string": "x-forwarded-for" }] },
              { "type": 3, "mapKey": [{ "type": 1, "string": "headerName" }], "mapValue": [{ "type": 1, "string": "x-real-ip" }] },
              { "type": 3, "mapKey": [{ "type": 1, "string": "headerName" }], "mapValue": [{ "type": 1, "string": "user-agent" }] }
            ]
...
function buildServerGtmTemplate(input) {
  ...
  const payload = {
    ...
    destinations,
    trackingDomain: cleanTemplateValue(input.trackingDomain, "https://track.yourdomain.com"),
    galleryTemplates: tagiooGalleryTemplateGuide(destinations.filter((destination) => !["meta", "tiktok"].includes(destination))),
    fieldMappings: {
      ...
      tiktok: destinations.includes("tiktok") ? "Tagioo TikTok Events API template and all-events server tag are included. Import server.json, preview, then publish." : "Not selected."
    }
  };
  ...
  if (destinations.includes("tiktok")) {
    tags.push(gtmTag(tags.length + 1, "Tagioo TikTok Events API - All Events", "cvt_0_102", [
      gtmTemplateParam("pixelId", "{{Tagioo - tiktok_pixel_id}}"),
      gtmTemplateParam("accessToken", "{{Tagioo - tiktok_access_token}}"),
      gtmBooleanParam("enableDebugLog", true)
    ], ["1"], "6"));
  }
  ...
  if (destinations.includes("meta")) customTemplates.push(gtmCustomTemplate(101, "Tagioo Meta CAPI", tagiooMetaCapiTemplateData()));
  if (destinations.includes("tiktok")) customTemplates.push(gtmCustomTemplate(102, "Tagioo TikTok Events API", tagiooTikTokEventsApiTemplateData()));
  if (customTemplates.length) content.customTemplate = customTemplates;

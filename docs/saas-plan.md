...
Email is the **automated** channel (Resend integration, `sendEmail` in [server.js](../server.js);
Brevo is wired as an alternative, used only when `RESEND_API_KEY` is unset).

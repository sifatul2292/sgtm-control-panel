...
Why Brevo was abandoned: transactional mail still did not send. **Root cause never confirmed** — two candidates were open when we reverted, and both are provider-independent, so the revert alone is unlikely to fix anything:
1. **DKIM was never published.** `brevo-code` TXT and `_dmarc` were live at tagioo.com's authoritative Cloudflare nameservers, but no DKIM under any selector probed (`mail`/`brevo`/`sib`/`s1`/`k1`/`default`._domainkey, TXT and CNAME). Brevo rejects sends from an unauthenticated domain. **Resend needs its own verified domain too** — publishing a DMARC record while having no signing key is worse than having neither.
2. **`.env` may never be loaded at all.** Nothing in this repo parses `.env` — no `dotenv` dep, no `--env-file`, no `process.loadEnvFile()` — and the documented start is a bare `pm2 start server.js --name tagioo` ([docs/tagioo-vps-setup.md:141](docs/tagioo-vps-setup.md)), which does not read `.env` either. If `pm2 env tagioo` shows no `RESEND_API_KEY`, transactional email has been silently failing since before any of this, and no provider swap can fix it. Fix would be `--node-args="--env-file=/var/www/tagioo/.env"`, and `pm2 restart --update-env` is required for env changes to take.
